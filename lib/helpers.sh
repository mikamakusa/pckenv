...
if [ -z "${PCKENV_CONFIG_DIR:-""}" ]; then
  PCKENV_CONFIG_DIR="$PCKENV_ROOT";
else
  PCKENV_CONFIG_DIR="${PCKENV_CONFIG_DIR%/}";
fi
export PCKENV_CONFIG_DIR;

if [ "${PCKENV_DEBUG:-0}" -gt 0 ]; then
  # Only reset DEBUG if PCKENV_DEBUG is set, and DEBUG is unset or already a number
  if [[ "${DEBUG:-0}" =~ ^[0-9]+$ ]] && [ "${DEBUG:-0}" -gt "${PCKENV_DEBUG:-0}" ]; then
    export DEBUG="${PCKENV_DEBUG:-0}";
  fi;
  if [[ "${PCKENV_DEBUG}" -gt 2 ]]; then
...
function load_bashlog () {
  source "${PCKENV_ROOT}/lib/bashlog.sh";
};
...
function curlw () {
  ...
  if [[ ! -z "${PCKENV_NETRC_PATH:-""}" ]]; then
    NETRC_OPT="--netrc-file ${PCKENV_NETRC_PATH}";
  else
    NETRC_OPT="";
  fi;
...
function check_active_version() {
  ...
  local active_version="$(${PCKENV_ROOT}/bin/packer ${maybe_chdir} version | grep '^packer')";
...
function check_installed_version() {
  local v="${1}";
  local bin="${PCKENV_CONFIG_DIR}/versions/${v}/packer";
...
function check_default_version() {
  local v="${1}";
  local def="$(cat "${PCKENV_CONFIG_DIR}/version")";
  [ "${def}" == "${v}" ];
};
...
source "$PCKENV_ROOT/lib/pckenv-exec.sh";
source "$PCKENV_ROOT/lib/pckenv-min-required.sh";
source "$PCKENV_ROOT/lib/pckenv-version-file.sh";
source "$PCKENV_ROOT/lib/pckenv-version-name.sh";

export pckenv_HELPERS=1;
